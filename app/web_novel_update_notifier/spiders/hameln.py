...
class HamelnSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if page_title == 'ログイン - ハーメルン':
            ...
            for div in response.xpath('//*[@id="main"]/div[2]/form/div').css('.section3'):

                title = div.xpath('h3/a[1]/text()').get().strip()
                author_link = div.xpath('h3/a[2]/text()')
                if author_link:
                    author = author_link.get().strip()
                else:
                    author = div.xpath('h3/text()').getall()[1].strip()[4:-1]
                novel_id = div.xpath('h3/a[1]/@href').get().strip().split('/')[-2]
                temp = div.xpath('p[2]/text()').get().strip()
                updated_at = datetime.datetime.strptime(temp[:11] + temp[15:], '%Y年%m月%d日%H:%M')
                latest_url = div.xpath('p[2]/a[1]/@href').get().strip()
